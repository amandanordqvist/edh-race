...
- Design north star: the whole site should feel like one drag pass (staging → launch → pass → chutes → partner invite); weigh choices against tyngd, precision, and återhållsamhet; reject scroll-dots, custom cursors, and glassmorphism cards; Journey/Resan is one interactive system — a running timeslip career arc (successes, setbacks, highlights), not a chapter list, chart, or filterable database; reject Journey filter chips, hero ET countdown, everyday-anchor comparison cards, and closing chart replay.
...
- GSAP powers scroll reveals, HomeStory pin+scrub, and the Journey timeslip cursor (`ScrollTrigger.scrub`, hold on the 2016 crossroads, reduced-motion snaps); Pass Arena is PlayCanvas on `/sv/passet` and `/en/pass` with a Home teaser only (no WebGL on Home), full 3D on mobile at lower fidelity; Camaro GLB at `public/models/pass/camaro.glb`; interactive chassis remains phase-2.
...
- Historical Journey/HomeStory plates live under `public/images/journey/`; HomeStory is a horizontal beat rail (desktop pin+scrub, mobile/reduced-motion snap); Journey/Resan uses `JourneyTimeslip` (`JourneySpine` removed) with structured `et`/`mph`/`kmh`/`distance`/`outcome` in `src/data/timeline.ts` (`TimelineWeight` is `regular`|`record`); Crossroads 2016 is a blank timeslip set piece; silent bookends at 2011–2012 and 2022; season bests/standings live in `src/data/results.ts`.
